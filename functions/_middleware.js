...
class UniversalHandler {
  ...
  element(element) {
    ...
    const node = { type: "element", tag: tagName, attributes: {}, children: [] };
    ...
    if (tagName === "include") {
      ...
      if (this.currentSlot) {
        ...
        if (fillTree) {
          let parent = this.elementStack.length > 1 ? this.elementStack[this.elementStack.length - 2] : null;
          if (parent) {
  ...
  text(textChunk) {
    if (this.currentSlot) {
      ...
      if (fillNode) {
        let parent = this.elementStack.length > 0 ? this.elementStack[this.elementStack.length - 1] : null;
        if (parent) {
  ...
  findParentInclude() {
    return [...this.elementStack].reverse().find(node => node.tag === "include")?.attributes?.path || null;
  }
  ...
  async processInclude(includePath) {
    ...
    try {
      ...
      let processedHTML = await new HTMLRewriter()
        ...
        .transform(new Response(includeText, { headers: { "content-type": "text/html" } }))
        .text();
